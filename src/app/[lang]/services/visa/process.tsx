        ...
        <div className="grid gap-8 md:grid-cols-3">
          {steps.map((step, index) => (
            <div key={index} className="relative flex flex-col items-center text-center">
              <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-r from-[#1289A6] to-[#1a8ba7]">
                <step.icon className="h-8 w-8 text-white" />
              </div>
              <h3 className="mb-4 text-xl font-bold text-zinc-900">{step.title}</h3>
              <p className="text-zinc-600">{step.description}</p>
              {index < steps.length - 1 && (
                <div className="absolute rtl:hidden left-1/2 top-8 hidden h-[2px] w-full -translate-y-1/2 bg-gradient-to-r from-[#1289A6] to-[#1a8ba7] md:block"></div>
              )}
            </div>
          ))}
        </div>
